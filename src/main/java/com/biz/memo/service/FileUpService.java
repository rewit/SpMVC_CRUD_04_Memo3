...
public class FileUpService {
	 ...
	 * 그 폴더만 외부에 공개를 할 예정
	 * 
	 * 실제 서버가 가지고 있는 resoruces 폴더의
	 * 위치를 알아야 할 필요가 있다
	 * 
	 * 실제 서버의 폴더위치를 추출하기 위해 
	 * ServletContext를 사용할 예정
	 */
	...
	public String fileUp(MultipartFile upFile) {
		...
		try {
			...
			//File dir = new File(fileDir,"tmp");
			File dir = new File(serverPath);
			
			//지금 내가 저장하기 위해 사용할 폴더(디렉토리)가 없는지
			if(!dir.exists()) {
				dir.mkdirs(); //없다면 만들어라
			}
			 ...
			 * 
			 * 그래서 서버에 저장할 파일 이름을 변환 시켜야 한다.
			 */
			
			/*
			 * UUID : 우주적 유일한 키값
			 * 파일이름 앞에 UUID 문자열을 추가하여
			 * 중복된 이름의 파일이 등록되는것을 막는다
			 */
			String uuString = UUID.randomUUID().toString();
			originalFileName = uuString + "_" + originalFileName;
			
			
			
			// 5. server에 저장할 폴더와 파일 이름을 결합한 객체 생성
			//File serverFile = new File(dir.getAbsolutePath(),originalFileName);
			File serverFile = new File(serverPath,originalFileName);
			
			//Java 1.8이하에서 보편적으로 사용하는 코드
			...
			//commons-io, commons-fileupload 를 사용하는 방법
			// 3, 6~9 번줄을 한 줄로 
